...
let pingAsync (target: IPAddress) = 
    async {
        ...
        let targetReply = pingSender.SendPingAsync(target)
...
let pingAll (targets: IPInfo[]) (router: IPInfo) = 
    let local = {IPAddress = IPAddress.Loopback; Name = "localhost"}
    let all = [|local; router; |] |> Array.append targets
    all
...
let pingAndLog (targetUrl: string) routerIP = 
    let router = { IPAddress = routerIP; Name = "router"}
    ...
    while true do
        let result = pingAll targets router
                     |> Array.map (fun i -> let info, reply = i
                                            pingToText info reply)
    
        let timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff",CultureInfo.InvariantCulture);
...
let main argv = 
    ...
    match routerIP with
    | None -> printfn "No Internet Connection"
              0
    | Some r -> if File.Exists(configFileName) then
